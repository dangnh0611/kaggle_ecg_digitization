...
class SignalFineHeatmapModel(nn.Module):
    ...
    def __init__(self, global_cfg):
        ...
        assert cfg.encoder_downsample == 2
        enc_strides = [e * cfg.encoder_downsample for e in enc_strides]

        logger.info(
            "Encoder multiscale feature (with downsample=%d) has channels=%s strides=%s",
            cfg.encoder_downsample,
            enc_channels,
            enc_strides,
        )

        # ========== FINE FEATURES ENCODER ==========
        # VGG19 to produce fine features
        # self.fine_encoder = timm.create_model(
        #     "vgg19.tv_in1k", pretrained=False, features_only=True, out_indices=[0, 1, 2], in_chans = 4
        # )
        self.fine_encoder = hydra.utils.instantiate(cfg.fine_encoder)
        fine_channels = self.fine_encoder.output_channels
        fine_strides = self.fine_encoder.output_strides
        logger.info(
            "Fine Encoder multiscale feature has channels=%s strides=%s",
            fine_channels,
            fine_strides,
        )
        # timeup! hard code for VGG19 here, make it generalize later
        assert fine_strides == [1, 2, 4]
        assert torch.equal(
            self.fine_encoder.normalize.mean, self.encoder.normalize.mean
        )
        assert torch.equal(self.fine_encoder.normalize.std, self.encoder.normalize.std)
        self.normalize = self.encoder.normalize
        del self.fine_encoder.normalize
        del self.encoder.normalize
        gc.collect()
        self.encoder.normalize = nn.Identity()
        self.fine_encoder.normalize = nn.Identity()

        # update encoder strides and channels
        assert enc_strides[1] == 8 and fine_strides[-1] == 4
        enc_strides = fine_strides + enc_strides[1:]
        # use stride 1/2/4 from fine encoder
        enc_channels = fine_channels + enc_channels[1:]

        logger.info(
            "GLOBAL MULTISCALE ENCODER has channels=%s strides=%s",
            enc_channels,
            enc_strides,
        )
    ...
    def forward(self, x):
        ...
        # since mean/std normalization is the same, don't do it again
        # just use normalized features (N, 3, H, W) directly
        x_half = F.interpolate(x_norm, scale_factor=0.5, mode="area")  # (N, 3, H/2, W/2)

        fine_features = self.fine_encoder(x_norm)
        # print("fine features:", [e.shape for e in fine_features])
        
        enc_features = self.encoder(x_half)[1:]
        # print("encoder features:", [tuple(e.shape) for e in enc_features])
        # strides: [1,2,4] + [8, 16, 32, 64]
        
        enc_features = fine_features + enc_features
        # print("combine features:", [e.shape for e in enc_features])
