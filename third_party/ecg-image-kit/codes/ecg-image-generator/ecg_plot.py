...
def ecg_plot(
    ...
    print('GLOBAL PLOT X/Y MIN/MAX:', x_min, x_max, y_min, y_max)
    ...
    for i in np.arange(len(lead_index)):
        ...
        if(show_lead_name):
            ...
            if (store_text_bbox):
                ...
                print('LEAD NAME BBOX:', leadName, x1, json_dict['height'] - y1, x2, json_dict['height'] - y2)
                box_dict = dict()
                x1 = int(x1)
                y1 = int(y1)
                x2 = int(x2)
        ...
        if(columns == 1 and i in np.arange(0,rows)):
            ...
            if(show_dc_pulse):
                ...
                print('DC_PULSE RAW', step, leadName, len(np.arange(0,sample_rate*standard_values['dc_offset_length']*step + 4*step,step)), np.arange(0,sample_rate*standard_values['dc_offset_length']*step + 4*step,step) + x_offset + x_gap,
                        dc_pulse+y_offset)
                if (bbox):
                    renderer1 = fig.canvas.get_renderer()
                    transf = ax.transData.inverted()
        ...
        print('Y OFFSET:', leadName, y_offset)
        ...
        if (bbox):
            ...
            if show_dc_pulse == False or (columns == 4 and (i != 0 and i != 4 and i != 8)):                                           
                ...
                x2 = bb.x1*resolution/fig.dpi
                print('PRE LEAD X0:', leadName, bb.x0*resolution/fig.dpi)
            box_dict = dict()
            print('LEAD', leadName, x1, y1, x2, y2)
            x1 = int(x1)
            y1 = int(y1)
            x2 = int(x2)
            y2 = int(y2)
        ...
        if columns > 1 and (i+1)%columns != 0:
            ...
            tmp_t = ax.plot(sep_x, sep_y, linewidth=line_width * 3, color=color_line)
            tbb = tmp_t[0].get_window_extent()
            x1, y1 = tbb.x0*resolution/fig.dpi, tbb.y0*resolution/fig.dpi
            x2, y2 = tbb.x1*resolution/fig.dpi, tbb.y1*resolution/fig.dpi
            print('SEP:', leadName, x1, json_dict['height'] - y1, x2, json_dict['height'] - y2)
    ...
    if(full_mode!='None'):
        ...
        if(show_lead_name):
            ...
            if (store_text_bbox):
                ...
                x2 = bb.x1*resolution/fig.dpi     
                y2 = bb.y1*resolution/fig.dpi
                # print('???', resolution / fig.dpi)  # == 1.0
                print('LEAD NAME BBOX:', full_mode, x1, json_dict['height'] - y1, x2, json_dict['height'] - y2)
                box_dict = dict()
                x1 = int(x1)
                y1 = int(y1)
                x2 = int(x2)
