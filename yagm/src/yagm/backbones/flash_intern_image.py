...
from yagm.utils.misc import load_state_dict
...
class DownsampleLayer(nn.Module):
    ...
    def __init__(self, channels, norm_layer="LN"):
        ...
        self.norm = build_norm_layer(
            2 * channels, norm_layer, "channels_first", "channels_first"
        )
        ...
        def _inner_forward(x, shape):
            if not self.layer_scale:
                if self.post_norm:
                    ...
                    x = x + self.drop_path(
                        self.res_post_norm1(self.dcn(self.norm1(x), shape))
                    )
                    x = x + self.drop_path(
                        self.res_post_norm2(self.mlp(self.norm2(x), shape))
                    )
...
class InternImageBlock(nn.Module):
    ...
    def __init__(
        ...
        self.downsample = (
            downsample_layer(channels=channels, norm_layer=norm_layer)
            if downsample
            else None
        )
...
class FlashInternImage(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # keep the correct layer ordering for better interpretation
        ...
        if mean is not None or std is not None:
            ...
            _C = mean.numel()
            assert _C == std.numel()
            mean = mean.reshape(1, _C, 1, 1).repeat(1, math.ceil(in_chans / _C), 1, 1)[
                :, :in_chans
            ]
            std = std.reshape(1, _C, 1, 1).repeat(1, math.ceil(in_chans / _C), 1, 1)[
                :, :in_chans
            ]
            logger.info(
                "Flash Intern Image normalization using\nMEAN=%s\nSTD=%s\n", mean, std
            )
        ...
        # load pretrain if needed
        if pretrained is not None:
            assert os.path.isfile(pretrained)
            # "model", "lr_scheduler", "max_accuracy", "epoch", "config", "amp"
            state_dict = torch.load(pretrained, weights_only=False)["model"]
            load_state_dict(self, state_dict, strict=False)
            logger.info("Loaded state dict from %s", pretrained)

    @property
    def output_strides(self):
        # stem has fixed stride of
        # first layer (x1) did not reduce resolution
        return [1] + [4 * 2**i for i in range(self.num_layers)]

    @property
    def output_channels(self):
        return [self.in_chans] + [self.channels * 2**i for i in range(self.num_levels)]
    ...
    def forward(self, x):
        if self.use_clip_projector:  # for InternImage-H/G
            ...
            features = self.forward_clip_projector(x)
        else:  # for InternImage-T/S/B/L/XL
            features = self.forward_features_seq_out(x)
        return features
