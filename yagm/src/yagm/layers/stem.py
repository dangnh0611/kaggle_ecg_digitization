...
from yagm.layers.common import SameConv1d, SameDepthwiseConv1d, make_divisible
...
class PatchEmbedStem1d(nn.Sequential):
...
class SqueezeformerStem1d(nn.Module):
    """Squeezeformer like"""
    ...
    def __init__(
        ...
        layers.extend(
            [
                SameDepthwiseConv1d(out_dim, out_dim, kernel_size=3, stride=1),
                nn.ReLU(),
            ]
            * (depth - 1)
        )
...
class ResnetV2Stem1d(nn.Module):
...
class ConvNeXtStem1d(nn.Module):
    ...
    def __init__(
        ...
        if stem_type == "patch":
            ...
            self.layers = PatchEmbedStem1d(
                in_dim, out_dim, norm_layer, stride, bias=True
            )
        else:
            mid_dim = (
...
def Stem1d(
    ...
    if stem_type == "patch":
        return PatchEmbedStem1d(in_dim, out_dim, norm_layer, stride, bias=True)
    elif stem_type == "squeeze":
        return SqueezeformerStem1d(in_dim, out_dim, kernel_size, stride, depth)
    elif stem_type in ["res", "deep", "deep_tiered"]:
        return ResnetV2Stem1d(
            in_dim, out_dim, norm_layer, kernel_size, stride, stem_type, preact
        )
    elif stem_type in ["overlap", "overlap_tiered"]:
        return ConvNeXtStem1d(
            in_dim, out_dim, norm_layer, kernel_size, stride, stem_type, preact
        )
    elif stem_type == "maxxvit":
        return MaxxvitStem1d(
