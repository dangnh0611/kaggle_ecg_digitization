...
import torch
from timm.scheduler.scheduler_factory import (
    create_scheduler_v2 as timm_create_scheduler_v2,
)
...
def resolve_scheduler_kwargs(cfg):
    ...
    total_steps = math.ceil(cfg.max_epochs * cfg.steps_per_epoch)
    assert total_steps - cfg.max_steps <= 1
    total_sched_steps = round(
        (cfg.max_epochs - cfg.cooldown_epochs) * cfg.steps_per_epoch
    )
    if cfg.warmup_steps is not None:
        assert cfg.warmup_epochs is None
        cfg.warmup_epochs = cfg.warmup_steps / cfg.steps_per_epoch
        warmup_steps = cfg.warmup_steps
    else:
        assert cfg.warmup_epochs is not None
        cfg.warmup_steps = round(cfg.warmup_epochs * cfg.steps_per_epoch)
        warmup_steps = cfg.warmup_steps
...
def create_scheduler(optimizer, cfg):
    ...
    if sched_name.startswith("transformers@"):
        ...
        scheduler, num_epochs = timm_create_scheduler_v2(
