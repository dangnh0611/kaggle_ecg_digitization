...
from yagm.transforms.onebumentations.serialization import (
    Serializable,
...
class BasicTransform(Serializable):
    ...
    def apply_with_params(
        ...
        for key, arg in kwargs.items():
            if arg is not None:
                ...
                output = target_function(arg, **dict(params, **target_dependencies))
    ...
    def __repr__(self) -> str:
        ...
        return "{name}({args})".format(
            name=self.__class__.__name__, args=utils.format_args(state)
        )
    ...
    def _get_target_validator(self, key: str) -> Callable:
        ...
        target_validator = self.target_validators.get(transform_key, lambda x, y: None)
...
class SequenceTransform(BasicTransform):
    ...
    @property
    def targets(self) -> Dict[str, Callable]:
        return {"seq": self.apply}

    def update_params(self, params: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        return params

    def apply(self, seq: np.ndarray, **params):
        raise NotImplementedError
    

class DualTransform(SequenceTransform):
    ...
    @property
    def targets(self) -> Dict[str, Callable]:
        return {"seq": self.apply, "keypoints": self.apply_keypoints}

    def apply(self, seq: np.ndarray, **params):
        raise NotImplementedError
    
    def apply_keypoints(self, keypoints: np.ndarray, **params):
        # simply unchanged
        return keypoints